...
def test_equipment_stats():
    ...
    assert_eq("attack with sword", eff["attack"], w.attack + 3)    # iron_sword +3
    assert_eq("defense with armor", eff["defense"], w.defense + 4) # steel_armor +4
...
def test_equipment_affects_damage():
    ...
    assert_gte("sword deals more or equal damage", out2["damage_taken"], out1["damage_taken"])


def test_no_equipment_returns_base_stats():
    w = make_warrior()
    eff = w.effective_stats()
    assert_eq("hp unchanged", eff["hp"], w.hp)
    assert_eq("speed unchanged", eff["speed"], w.speed)
...
if __name__ == "__main__":
    run_test(
        ...
        test_no_equipment_returns_base_stats,
    )